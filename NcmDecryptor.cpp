...
NcmDecryptor::NcmDecryptor(const std::vector<uint8_t>& data, const CString& filename)
...
DecryptResult NcmDecryptor::Decrypt()
{
...
std::vector<uint8_t> NcmDecryptor::GetKeyData()
{
...
std::vector<uint8_t> NcmDecryptor::GetKeyBox()
{
...
NcmMusicMeta NcmDecryptor::GetMetaData()
{
    if (m_offset + 4 > m_raw.size())
...
std::vector<uint8_t> NcmDecryptor::GetAudio(const std::vector<uint8_t>& keyBox)
{
...
CString NcmDecryptor::Utf8ToWstring(const CString& s)
{
    if (s.IsEmpty())
        return {};
